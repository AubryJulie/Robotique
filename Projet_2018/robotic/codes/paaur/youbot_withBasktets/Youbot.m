classdef Youbot
    ...
    properties (Constant)
        INFLATE_INIT = .45; % Inflate factor used for preparing maps for frontier-based approach 
    ...
    properties (Access = private)
        ...
        flagRecovery % Flag 1 if in recovery state, 0 otherwise
        resolution % resolution of map is number of cells per meter
        flagMapping %  Flag 1 if mapping needs to be done, 0 otherwise
        startPose % Memorise initial pose
        baskets % center of baskets & access points coordinates & 0-1-2: 0 if not visited, 1 is in visit, 2 is photographed
        tables % center coordinates of tables with objects
    end
    ...
    methods (Access = public)
        ...
        function obj = Youbot(app, resolution)
            ...
            obj.startPose = obj.x; % Memorize initial pose
            ...
            while obj.app.onoff.Value == 1 % Flag in UI controlled by start/stop button
                ...
                if (mod(obj.counter,obj.T_UI) == 0)
                    ...
                    if obj.flagMapping
                        ...
                        a = world2grid(obj.m, obj.x(1:2));
                        domain(a(1),a(2)) = 0.1;
                        if ~isempty(obj.goal) || ~isempty(obj.baskets)
                            b = world2grid(obj.m, obj.baskets(:,1:2));
                            c = world2grid(obj.m, obj.goal);
                            domain(b(:,1),b(:,2)) = 0.2;
                            domain(c(1),c(2)) = 0.3;
                        end
                        ...
                        [occX, occY] = find(domain ~= 0);
                        [aY, aX] = find(domain == 0.1);
                        [bY, bX] = find(domain == 0.2);
                        [cY, cX] = find(domain == 0.3);
                    end
                    
                    if obj.flagMapping == 0
                        plot(an, occY, occX, 'k+', aX, aY, 'r*', bX, bY, 'go', cX, cY, 'b*');    
                    elseif isempty(obj.frnts)
    ...
    methods (Access = private)
        ...
        function obj = state_goal(obj)
            if obj.flagMapping % In exploration mode
                obj = obj.compute_goal(obj.x(1:2)); % Compute goal
            else % In basket mode
                obj.goal = [];
                min_dist = Inf;
                for i = 1:size(obj.baskets,1) 
                    access = obj.baskets(i,3:4);
                    flagVisited = obj.baskets(i,5);
                    dist = norm(access-obj.x(1:2));
                    if min_dist > dist && flagVisited == 0
                        obj.goal = access;
                    end
                end
                if ~isempty(obj.goal)
                    id1 = find(obj.baskets(:,3) == obj.goal(1));
                    id2 = find(obj.baskets(:,4) == obj.goal(2));
                    obj.baskets(intersect(id1,id2),5) = 1;
                else 
                    obj.fsm = 'finish';
                    obj.prevfsm = 'goal';
                    obj.UI_update_state(); % Update state
                    disp('[LOG] Basket finished');
                end
            end
...
%             obj = obj.mapping(); % Update mapping with laser scans
        ...
        function obj = state_move(obj)
            ...
            if d > obj.GOAL_RADIUS 
                ...
                if obj.counter > obj.counterHelper + 30
                    ...
                    if obj.flagMapping == 0 % Reached basket access point
                        idx = find(obj.baskets(:,5) == 1);
                        % TODO turn & photo
                        obj.baskets(idx,5) = 2;
                    end
                end
        ...
        %% Transition between mapping and basket mode 
        function obj = state_transition(obj)
            mat = obj.m.occupancyMatrix();
            obj.baskets = find_baskets(mat);
            temp1 = grid2world(obj.m, int16(obj.baskets(:,1:2)));
            temp2 = grid2world(obj.m, int16(obj.baskets(:,3:4)));
            obj.baskets = [temp1 temp2];
            obj.baskets = [obj.baskets zeros(size(obj.baskets,1), 1)];
            obj.tables = find_tables(mat, world2grid(obj.m, obj.startPose(1:2)), obj.startPose(3));
            obj.flagComputeGoal = 1;
            
            obj.fsm = 'control';
            obj.prevfsm = 'transition';
        ...
        function obj = state_finish(obj)
            ...
            obj.leftRightVel = 0;
            obj.rotVel = 0;
            if obj.flagMapping % Not gone through the basket mode
                obj.flagMapping = 0; % Mapping is done
                obj.baskets = obj.x(1:2);
                obj.goal = obj.x(1:2);
                obj.fsm = 'transition';
                obj.prevfsm = 'finish';
                disp('[LOG] Enter basket mode');
            else 
                obj.app.onoff.Value = 0; % Stop 
                disp('[LOG] Finish');
            end
            % Update UI
            obj.UI_update_state();
        ...
        function obj = compute_goal(obj, start)
            ...
            if isempty([r, c]) % No points to explore anymore
                obj.fsm = 'finish';
                obj.UI_update_state();
                obj.goal = [];
            ...
            if isempty(cst<0.5) % No points to explore anymore
                obj.fsm = 'finish';
                obj.UI_update_state();
                obj.goal = [];
