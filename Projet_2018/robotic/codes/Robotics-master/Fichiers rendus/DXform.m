...
%Added by pierre nicolay
% distancemap_get return the distance map
% distancemap_set set the distance map
...
classdef DXform < Navigation
    ...
    methods
        ...
        function plan(dx, goal, show)
            ...
            %% Modification from Pierre Nicolay
            %set a sqaure arround the robot
            try
            dx.occgrid(goal(2)-3:goal(2)+3,goal(1)-3:goal(1)+3) = 0;
            catch
            end
            %end of mofication
        ...
        function distancemap_set(dx,dmap)
           dx.distancemap = dmap; 
        end
        
        function dmap = distancemap_get(dx)
           dmap = dx.distancemap;
        end
