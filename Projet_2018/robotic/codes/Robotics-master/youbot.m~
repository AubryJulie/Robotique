function youbot()
    ...
    if plotData
        ...
        [X, Y] = meshgrid(-7.75:.25:7.75, -7.75:.25:7.75);
        X = reshape(X, 1, []);
        Y = reshape(Y, 1, []);
    end
    ...
    while true
        ...
        %!! PlotData is supposed to be true !!
        if strcmp(fsm,'rotate') || strcmp(fsm,'drive')


        [pts, contacts] = youbot_hokuyo(vrep, h, vrep.simx_opmode_buffer);
        rotmZYZ = eul2r(youbotEuler,'ZYZ');

        absoluteContacts(:,index:index +length(pts(1,contacts))-1)= rotmZYZ*pts(:,contacts) + repmat(transpose(youbotPos),1,length(pts(:,contacts)));
        %absolutePath(:,index:index + lengeth(pts(1,:))-1) = rotmZYZ*pts +  repmat(transpose(youbotPos),1,length(pts(:,contacts)));
        index = index +length(pts(1,contacts));

        position(1,index2) = youbotPos(1);
        position(2,index2) = youbotPos(2);
        z(index2) = youbotEuler(3);
        index2 = index2+1;
        ...
        if plotData
            ...
            % sensor position.
            %subplot(211)
            %plot(X(in), Y(in), '.g',... % Green surface
            %     pts(1, contacts), pts(2, contacts), '*r',... % Contact with objects
            %     [h.hokuyo1Pos(1), pts(1, :), h.hokuyo2Pos(1)], [h.hokuyo1Pos(2), pts(2, :), h.hokuyo2Pos(2)], 'r',... % Field of view limits
            %     0, 0, 'ob',... % Rear of the robots
            %     h.hokuyo1Pos(1), h.hokuyo1Pos(2), 'or',... % Sensor's position
            ...
            %plot(position(1,:)+0.1*sin(z),position(2,:)-0.1*cos(z),'r*');
            plot(absoluteContacts(1,:),absoluteContacts(2,:),'*');
            %figure;
            %plot(absolutePath(1,:),absoluteContacts(2,:),'.g');
            %axis equal;
            drawnow;
        ...
        elseif strcmp(fsm, 'drive')
            ...
            if (youbotPos(1) + 5.167 < .001) && (abs(youbotPos(1) - prevLoc) < .001)
                forwBackVel = 0;
