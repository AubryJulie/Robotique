                            ...
                            if w > length(Panier5)
                                if w > length(Panier6)
                                    ...
                                    if w > length(Panier7)
                                        ...
                                        Table1 = Panier7;
                                        T = [length(Panier2) length(Panier3) length(Panier4)...
                                            length(Panier5) length(Panier6) length(BrowniesA)];
                                    end
                                end                
                            else
                                w = length(Panier5);
...
%                 fsm ='grasp';
                ...
                l2 = 0.155; % length of the first arm 
                l3 = 0.135; % length of the second arm 
                l4 = 0.145;%length of the third arm
            ...
            elseif strcmp(fsm, 'findbasket')
                ...
                %NewGoal = basketGoal(basket_number,:)
                ...
                if strcmp (formBlock, inst(i).shape)
                    if strcmp (colorName,inst(i).colorname)
                        ...
                        [label, ~] = predict(categoryClassifier1, image);
            ...
            elseif strcmp(fsm, 'basket_rotate')
             ...
             elseif strcmp(fsm, 'basket_slide')
                ...
                indexBasket = find(bsxfun(@hypot, pts_basket(:,1)- youbotPos(1), pts_basket(:,2)-youbotPos(2))==...
                    min(bsxfun(@hypot, pts_basket(:,1)-youbotPos(1), pts_basket(:,2)-youbotPos(2))),1,'first');
...
%                 
               ...
               AnglDestBlock = atan((basketCenter(counter,2) - (resol*youbotPos(2)-minMap(2)+1))/...
                    (basketCenter(counter,1) - (resol*youbotPos(1)-minMap(1)+1))) ;
                if (basketCenter(counter,1) - (resol*youbotPos(1)-minMap(1)+1)) > 0
                    AnglDestBlock = AnglDestBlock + pi;
                end

                blockClosePoint = [(youbotPos(1) - (basketDist - 0.3) * cos(AnglDestBlock))...
                    (youbotPos(2) - (basketDist - 0.3) * sin(AnglDestBlock))];
            ...
            elseif strcmp(fsm, 'basket_movearm')
                ...
                gripPos = homtrans(trf, transpose(gripPos)); 
                 ...
                 % rotation to the block on the table
                 NewAngl = atan(((resol*gripPos(1)-minMap(1)+1) - basketCenter(counter,1))/...
                     (basketCenter(counter,2) - (resol*gripPos(2)-minMap(2)+1)))- youbotEuler(3);
                 if (basketCenter(counter,2) - (resol*gripPos(2)-minMap(2)+1)) > 0
                     NewAngl = NewAngl + pi;
                 end
            ...
            elseif strcmp(fsm, 'basket_movearm2')
                ...
                res = vrep.simxSetJointTargetPosition(id, h.armJoints(2), 1.3, vrep.simx_opmode_oneshot);
                vrchk(vrep, res, true);
                pause(2);
