...
while true
    ...
    if strcmp(fsm, 'rotate')
        ...
        if rotation_start
            abs(youbotPos(1) - next_position_x)
            abs(youbotPos(2) - next_position_y)
            if abs(youbotPos(1) - next_position_x) < abs(youbotPos(2) - next_position_y)
                rotate_y = true;
            else
                rotate_y = false;
            end
        ...
        if (abs(angdiff(angle, youbotEuler(3))) < .01 / 180 * pi)
            rotateRightVel = 0;
            fsm = 'mapping';
            storing = true;
            int_mapping = true;
    ...
    elseif strcmp(fsm, 'motion')
        ...
        if indice == length(new_traj) && distance < 0.5
            ...
            fsm = 'mapping';
            indice = 1;
        elseif (distance < .01) && indice ~= length(new_traj)
            forwBackVel = 0;
            fsm = 'mapping';
            rotation_start = true;
            indice = indice + 1;
    ...
    elseif strcmp(fsm, 'mapping')
        ...
        for i = 1:l_X
            ...
            % don't free blocked_cells.
            if(plan(uint8(10*new_X(i)+len/2),uint8(10*new_Y(i)+width/2)) ~= blocked_cell)
