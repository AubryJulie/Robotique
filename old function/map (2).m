...
while true
    ...
    if strcmp(fsm, 'rotate')
        ...
        distance = sqrt( (next_position_x - youbotPos(1) )^2 + (next_position_y - youbotPos(2) )^2 );
        angle = acos( (next_position_x - youbotPos(1) )/distance );
        angle = angle*180/pi;
    ...
    elseif strcmp(fsm, 'mapping')
        ...
        for i = 1:l_X
			% If the map is too small in x direction, it is extend.
			...
			% If the map is too small in y direction, it is extend.
        ...
        point = pts(:,contacts);
        l_p = length(point);
		% Fill the map with blocked cells.
        for i=1:l_p
            x = point(1,i);
            y = point(2,i);
			% If the map is too small in x direction, it is extend.
    ...
    elseif strcmp(fsm, 'trajectory')
		% traj_full contain all the steps needed to reach the target.
        ...
        y_previous = traj_full(1).y;
        y_current = traj_full(2).y;
        if x_previous == x_current
            direction = 1;
        elseif y_previous == y_current
            direction = 2;
        elseif y_previous - y_current == x_previous - x_current
            direction = 3;
        else
            direction = 4;
        end
